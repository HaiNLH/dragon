...
class RTG(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.drop_node_idx = self.dropv_node_idx + self.dropt_node_idx
        ...
        self.edge_index_drop = torch.cat((self.edge_index_dropv, self.edge_index_dropt), dim=1 )
        ...
        if self.v_feat is not None and self.t_feat is not None and self.concat_feat is not None:
            self.concat_drop_ze = torch.zeros(len(self.drop_node_idx), self.t_feat.size(1)).to(self.device)
            self.concat_gcn = GCN(self.dataset, batch_size, num_user, num_item, dim_x, self.aggr_mode,
                                  num_layer=self.num_layer, has_id=has_id, dropout=self.drop_rate, dim_latent=64,
                                  device=self.device, features=self.concat_feat)
    ...
    def forward(self, interaction):
        ...
        # Concat feature and get new representation
        if self.v_feat is not None and self.t_feat is not None:
        ...
        if self.concat_feat is not None:
            self.concat_rep, self.concat_pref = self.concat_gcn(self.edge_index_drop, self.edge_index, self.concat_feat)
            representation = self.concat_rep
        ...
        #             representation = torch.cat((self.t_rep, self.v_rep), dim=1)
        #         else:
        #             representation += self.t_rep
        ...
        if self.construction == 'cat':
            ...
            if self.concat_rep is not None:
                user_rep = self.concat_rep[:self.num_user]
        ...
        item_rep = item_rep + h
        #item_rep_resized = item_rep.view(128, -1)
